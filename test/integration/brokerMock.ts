...
server.get("/pacts/provider/nolinks/latest", (req, res) => {
	...
	const obj = JSON.parse(`{ "_links": { "self": { "href": "${BROKER_HOST}/pacts/provider/nolinks/latest/sit4", "title": "Latest pact versions for the provider nolinks with tag \"sit4\"" }, "provider": { "href": "${BROKER_HOST}/pacticipants/nolinks", "title": "bobby" }, "pacts": [] } }`);
	res.json(obj);
});
