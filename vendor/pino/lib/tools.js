...
function asJson(obj, msg, num, time) {
	...
	let value;
	let notHasOwnProperty = obj.hasOwnProperty === undefined;
	if (formatters.log) {
...
function getPrettyStream(opts, prettifier, dest, instance) {
	...
	try {
		let prettyFactory = require('pino-pretty');
	...
	return {
		...
		write(chunk) {
			...
			const formatted = pretty(
				typeof redact === 'function' ? redact(obj) : obj,
			);
...
function createArgsNormalizer(defaultOptions) {
	return function normalizeArgs(instance, opts = {}, stream) {
		...
		if ('extreme' in opts) {
			throw Error(
				'The extreme option has been removed, use pino.destination({ sync: false }) instead',
			);
		}
		...
		if (
			...
			stream.fd >= 0 &&
			!hasBeenTampered(stream)
		) {
			console.log('-------------------------- using stdout');
			stream = buildSafeSonicBoom({ fd: stream.fd, sync: true });
		}
		if (prettyPrint) {
			console.log('-------------------------- using prettyPrint');
...
function final(logger, handler) {
	...
	if (typeof stream.flushSync !== 'function') {
		throw Error(
			'final requires a stream that has a flushSync method, such as pino.destination',
		);
	}
